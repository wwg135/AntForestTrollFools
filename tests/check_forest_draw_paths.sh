...
chk "诊断含安全可做"             "$M" '安全可做 %ld'
chk "诊断含限时类"               "$M" '限时类 %ld'
chk "诊断含今日已完成缓存"        "$M" '今日已完成缓存 %ld'
chk "诊断含今日失败缓存"          "$M" '今日失败缓存 %ld'
chk "诊断含逐条明细"             "$M" '明细：%@'
chk "明细用中文标题（便于对账）"   "$M" 'NSString *label = title.length ? title : taskType;'
chkno "旧『最大数组』口径已删"     "$M" 'best.count > '
chkno "旧『待做(TODO)』口径已删"   "$M" '待做(TODO) %ld'
