...
echo "[1/10] Checking farm state is read from farmVO.subFarmVO (real state packet shape)..."
...
echo "[2/10] Checking candidate pick prefers the subFarmVO that really carries foodInTrough..."
...
echo "[3/10] Checking empty subFarmVO ({}) is not treated as a state packet..."
grep -Fq 'if ([(NSDictionary *)cand count] == 0) continue;' "$source_file"

echo "[4/10] Checking missing trough field != zero (manorTroughKnown gate)..."
...
echo "[5/10] Checking server feed status (animalFeedStatus=EATING) is the authoritative eating signal..."
...
grep -Fq 'static NSDictionary *manorChickenStatusVO(NSDictionary *ownAnimal, NSDictionary *subFarm, NSDictionary *innerSub) {' "$source_file"
grep -Fq 'ownAnimal[@"animalStatusVO"]' "$source_file"
grep -Fq 'manorChickenStatusVO(ownAnimal, subFarm, innerSub)[@"animalFeedStatus"]' "$source_file"
...
echo "[6/10] Checking unknown trough defers feeding and pulls authoritative state instead..."
...
echo "[7/10] Checking the old blind-feed writing does not come back..."
...
echo "[8/10] Checking backpack stock is read from farmVO.foodStock (enterFarm packet puts it there)..."
grep -Fq 'id stockRaw = subFarm[@"foodStock"] ?: resData[@"foodStock"] ?: dict[@"foodStock"] ?: farmVO[@"foodStock"] ?: innerSub[@"foodStock"];' "$source_file"
grep -Fq 'id stockLimitRaw = subFarm[@"foodStockLimit"] ?: resData[@"foodStockLimit"] ?: farmVO[@"foodStockLimit"] ?: innerSub[@"foodStockLimit"];' "$source_file"
grep -Fq 'if (stockFromPacket) {' "$source_file"
if grep -Fq 'if (subFarm[@"foodStock"]) {' "$source_file"; then
    echo "❌ 旧的背包存量取值链（漏 farmVO）复活了"
    exit 1
fi

echo "[9/10] Checking missing backpack-stock field is NOT treated as 0 (falls back to last synced value)..."
grep -Fq 'static BOOL gManorFoodStockKnown = NO;' "$source_file"
grep -Fq 'id stockRaw = subFarm[@"foodStock"] ?: resData[@"foodStock"] ?: dict[@"foodStock"] ?: farmVO[@"foodStock"] ?: innerSub[@"foodStock"];' "$source_file"
grep -Fq 'BOOL stockFromPacket = (stockRaw != nil);' "$source_file"
grep -Fq 'gManorFoodStockKnown = YES;' "$source_file"
grep -Fq 'foodStock = self.lastManorFoodStock;' "$source_file"
grep -Fq '蚂蚁庄园：背包存量还没同步到，本轮暂缓投喂（不盲喂）' "$source_file"
grep -Fq 'recordEggDiagOnce(self, @"stock_packet_missing",' "$source_file"
if grep -Fq '诊断 · 本状态包未带背包存量（沿用上次同步值 %ldg）｜顶层键=%@' "$source_file"; then :; else
    echo "❌ 缺字段诊断行缺失（下次出现同类问题无法定位形态）"
    exit 1
fi

echo "[10/10] Checking version bump (informational) + no new independent switch..."
...
if [ "$entry_ver" != "v3.2.6" ]; then
    echo "⚠️ 面板版本号不是 v3.2.6（实际：${entry_ver:-空}）"
