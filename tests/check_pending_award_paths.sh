...
echo "[1/24] Checking award unit splits by awardType (ALLPURPOSE=g / CUISINE=个)..."
...
echo "[2/24] Checking pending-feed total helper scans the whole list with the SAME claimability rule..."
...
echo "[3/24] Checking today's sign award is NOT counted into the pending total..."
...
echo "[4/24] Checking the food-stock cap gate only applies to g-unit awards..."
grep -Fq 'if (isFoodAward && limit > 0 && stock + award > limit) {' "$source_file"

echo "[5/24] Checking panel log shows the task-only total with count..."
grep -Fq '待领合计 %ldg（%lu 个已完成任务）' "$source_file"
grep -Fq '正在领取 %ld%@ 奖励（预估容量 %ldg/%ldg）...' "$source_file"

echo "[6/24] Checking the old single-task / unit-blind writing does not come back..."
...
echo "[7/24] Checking 庄园 claimability covers FINISHED + CAN_RECEIVE/WAIT_AWARD/... + 「领取」按钮..."
...
echo "[8/24] Checking the claim site no longer claims by FINISHED-only (regression guard)..."
...
echo "[9/24] Checking the 庄园 task-list diagnostic (对账用) is wired in..."
...
grep -Fq 'manorDiag = [NSString stringWithFormat:@"%@ · 背包 %ldg/%ldg%@", manorDiag, (long)diagStock, (long)diagLimit,' "$source_file"
grep -Fq '（已近上限，领奖会挂起等腾空后补领）' "$source_file"

echo "[10/24] Checking the claim-receipt fallback retry (记账后无回执 → 允许再试，封顶 2 次)..."
...
echo "[11/24] Checking the reward-bridge waiting log is throttled (不再每轮刷屏)..."
...
echo "[11b/24] Checking 睡觉/喂鸡修复没有把任务查询(领奖)链截断..."
...
echo "[12/24] Checking version sync (informational) + no new independent switch..."
...
echo "[13/24] Checking claim accounting is persisted (must survive app restart)..."
...
echo "[14/24] Checking a billed-but-never-sent claim is re-claimed instead of silently skipped..."
grep -Fq 'if (sent <= 0) {' "$source_file"
grep -Fq '今日有领奖记账但查不到发送记录，按未领取补领一次' "$source_file"

echo "[15/24] Checking waiting / retry-limit claims always leave a log line (no silent skip)..."
grep -Fq '今日已记账领奖（%@），本轮跳过' "$source_file"
grep -Fq '"manor_claim_wait:%@"' "$source_file"

echo "[16/24] Checking receipt wait is bounded (<=120s, else a whole day can pass without a retry)..."
...
echo "[17/24] Checking server-state receipt (RECEIVED) confirms delivery once and clears the wait record..."
grep -Fq '奖励已到账（服务端状态 RECEIVED）' "$source_file"
grep -Fq '"manor_claim_ok:%@"' "$source_file"

echo "[18/24] Checking the claim reply is traced (was silently dropped by the status-packet router)..."
grep -Fq '蚂蚁庄园 · 领取回包（taskId=%@）' "$source_file"
grep -Fq 'isClaimReply' "$source_file"

echo "[19/24] Checking a 20s no-reply watchdog exists for the claim request..."
grep -Fq '领取请求已发出 20 秒仍未见回包' "$source_file"

echo "[20/24] Checking a reply marks the receipt (no blind retry, no legacy re-claim loop)..."
grep -Fq 'ANTFARM_CLAIM_REPLY:' "$source_file"
grep -Fq 'manorClaimHasReply(taskId)' "$source_file"
grep -Fq '今日已收到领取回包，不再重复领取' "$source_file"

echo "[21/24] Checking the claim submit log carries taskId + op (so the reply can be matched)..."
grep -Fq '（taskId=%@, op=receiveFarmTaskAward）' "$source_file"

echo "[22/24] Checking the 诊断行 only prints on change (不再每轮重复同一条 39 任务明细)..."
grep -Fq 'static NSString *lastManorDiagLine = nil;' "$source_file"
grep -Fq 'if (![manorDiag isEqualToString:lastManorDiagLine] || ![diagDay isEqualToString:lastManorDiagDay]) {' "$source_file"
if grep -Fq 'if (claimableInList > 0) [self recordStage:manorDiag];' "$source_file"; then
    echo "❌ 每轮无条件重复打印诊断行的旧写法复活了"
    exit 1
fi

echo "[23/24] Checking any bound bridge can serve the generic gateway chains (进一个页面即可跑另外两家网关任务)..."
...
echo "[24/24] Checking the cross-page guard now leaves a log line (不再静默 return)..."
