...
echo "[9/28] Checking the 庄园 task-list diagnostic (对账用) is wired in..."
...
grep -Fq 'NSString *manorDiagCore = [NSString stringWithFormat:@"%@ · 背包 %ldg/%ldg%@", manorDiag, (long)diagStock, (long)diagLimit,' "$source_file"
...
echo "[11b/28] Checking 睡觉/喂鸡修复没有把任务查询(领奖)链截断..."
...
echo "[12/28] Checking version sync (informational) + no new independent switch..."
...
echo "[13/28] Checking claim accounting is persisted (must survive app restart)..."
...
echo "[14/28] Checking a billed-but-never-sent claim is re-claimed instead of silently skipped..."
grep -Fq 'if (sent <= 0) {' "$source_file"
grep -Fq '今日有领奖记账但查不到发送记录，按未领取补领一次' "$source_file"

echo "[15/28] Checking waiting / retry-limit claims always leave a log line (no silent skip)..."
grep -Fq '今日已记账领奖（%@），本轮跳过' "$source_file"
grep -Fq '"manor_claim_wait:%@"' "$source_file"

echo "[16/28] Checking receipt wait is bounded (<=120s, else a whole day can pass without a retry)..."
...
echo "[17/28] Checking server-state receipt (RECEIVED) confirms delivery once and clears the wait record..."
grep -Fq '奖励已到账（服务端状态 RECEIVED）' "$source_file"
grep -Fq '"manor_claim_ok:%@"' "$source_file"

echo "[18/28] Checking the claim reply is traced (was silently dropped by the status-packet router)..."
grep -Fq '蚂蚁庄园 · 领取回包（taskId=%@）' "$source_file"
grep -Fq 'isClaimReply' "$source_file"

echo "[19/28] Checking a 20s no-reply watchdog exists for the claim request..."
grep -Fq '领取请求已发出 20 秒仍未见回包' "$source_file"

echo "[20/28] Checking a reply marks the receipt (no blind retry, no legacy re-claim loop)..."
grep -Fq 'ANTFARM_CLAIM_REPLY:' "$source_file"
grep -Fq 'manorClaimHasReply(taskId)' "$source_file"
grep -Fq '今日已收到领取回包，不再重复领取' "$source_file"

echo "[21/28] Checking the claim submit log carries taskId + op (so the reply can be matched)..."
grep -Fq '（taskId=%@, op=receiveFarmTaskAward）' "$source_file"

echo "[22/28] Checking the 诊断行 only prints on change (不再每轮重复同一条 39 任务明细)..."
grep -Fq 'static NSString *lastManorDiagLine = nil;' "$source_file"
grep -Fq 'if (![manorDiagCore isEqualToString:lastManorDiagLine] || ![diagDay isEqualToString:lastManorDiagDay]) {' "$source_file"
if grep -Fq 'if (claimableInList > 0) [self recordStage:manorDiag];' "$source_file"; then
    echo "❌ 每轮无条件重复打印诊断行的旧写法复活了"
    exit 1
fi
...
echo "[27/28] Checking the diagnostic stock value is stamped with its sync time (背包值新鲜度)..."
grep -Fq 'gManorFoodStockSyncedAt = [[NSDate date] timeIntervalSince1970];' "$source_file"
grep -Fq '（状态包同步于 %@）' "$source_file"
grep -Fq 'if (![manorDiagCore isEqualToString:lastManorDiagLine] || ![diagDay isEqualToString:lastManorDiagDay]) {' "$source_file"
