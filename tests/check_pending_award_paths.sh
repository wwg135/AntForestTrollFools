...
echo "[1/27] Checking award unit splits by awardType (ALLPURPOSE=g / CUISINE=个)..."
...
echo "[2/27] Checking pending-feed total helper scans the whole list with the SAME claimability rule..."
...
echo "[3/27] Checking today's sign award is NOT counted into the pending total..."
...
echo "[4/27] Checking the food-stock cap gate only applies to g-unit awards..."
grep -Fq 'if (isFoodAward && limit > 0 && stock + award > limit) {' "$source_file"

echo "[5/27] Checking panel log shows the task-only total with count..."
grep -Fq '待领合计 %ldg（%lu 个已完成任务）' "$source_file"
grep -Fq '正在领取 %ld%@ 奖励（预估容量 %ldg/%ldg）...' "$source_file"

echo "[6/27] Checking the old single-task / unit-blind writing does not come back..."
...
echo "[7/27] Checking 庄园 claimability covers FINISHED + CAN_RECEIVE/WAIT_AWARD/... + 「领取」按钮..."
...
echo "[8/27] Checking the claim site no longer claims by FINISHED-only (regression guard)..."
...
echo "[9/27] Checking the troubleshooting 任务诊断 is removed (排查用，已按用户要求删除)..."
if grep -Fq 'manorTaskListDiag' "$source_file"; then
    echo "❌ 排查用的任务诊断又回来了（用户已要求删除）"
    exit 1
fi

echo "[10/27] Checking the claim-receipt fallback retry (记账后无回执 → 允许再试，封顶 2 次)..."
...
echo "[11/27] Checking the reward-bridge waiting log is throttled (不再每轮刷屏)..."
...
echo "[12/27] Checking 睡觉/喂鸡修复没有把任务查询(领奖)链截断..."
...
echo "[13/27] Checking version sync (informational) + no new independent switch..."
...
echo "[14/27] Checking claim accounting is persisted (must survive app restart)..."
...
echo "[15/27] Checking a billed-but-never-sent claim is re-claimed instead of silently skipped..."
grep -Fq 'if (sent <= 0) {' "$source_file"
grep -Fq '今日有领奖记账但查不到发送记录，按未领取补领一次' "$source_file"

echo "[16/27] Checking waiting / retry-limit claims always leave a log line (no silent skip)..."
grep -Fq '今日已记账领奖（%@），本轮跳过' "$source_file"
grep -Fq '"manor_claim_wait:%@"' "$source_file"

echo "[17/27] Checking receipt wait is bounded (<=120s, else a whole day can pass without a retry)..."
...
echo "[18/27] Checking server-state receipt (RECEIVED) confirms delivery once and clears the wait record..."
grep -Fq '奖励已到账（服务端状态 RECEIVED）' "$source_file"
grep -Fq '"manor_claim_ok:%@"' "$source_file"

echo "[19/27] Checking the claim reply is traced (was silently dropped by the status-packet router)..."
grep -Fq '蚂蚁庄园 · 领取回包（taskId=%@）' "$source_file"
grep -Fq 'isClaimReply' "$source_file"

echo "[20/27] Checking a 20s no-reply watchdog exists for the claim request..."
grep -Fq '领取请求已发出 20 秒仍未见回包' "$source_file"

echo "[21/27] Checking a reply marks the receipt (no blind retry, no legacy re-claim loop)..."
grep -Fq 'ANTFARM_CLAIM_REPLY:' "$source_file"
grep -Fq 'manorClaimHasReply(taskId)' "$source_file"
grep -Fq '今日已收到领取回包，不再重复领取' "$source_file"

echo "[22/27] Checking the claim submit log carries taskId + op (so the reply can be matched)..."
grep -Fq '（taskId=%@, op=receiveFarmTaskAward）' "$source_file"

echo "[23/27] Checking any bound bridge can serve the generic gateway chains (进一个页面即可跑另外两家网关任务)..."
...
echo "[24/27] Checking the cross-page guard now leaves a log line (不再静默 return)..."
...
echo "[25/27] Checking the startup grace period for the bridge-waiting log (启动瞬间误报不再提示)..."
...
echo "[26/27] Checking the claim reply is never swallowed + always traced (只记不吞 + 兜底留痕)..."
...
echo "[27/27] Checking the claim-reply trail survives the rolling log window (落盘 + 启动补打)..."
