...
echo "[1/6] Checking one-shot wake timer exists (generation + absolute time, no runloop ownership)..."
...
echo "[2/6] Checking wake is scheduled from the state packet's countdown (feeding remaining seconds)..."
...
echo "[3/6] Checking wake refreshes authoritative state instead of blind-feeding..."
...
echo "[4/6] Checking repeat packets do not reschedule and the wake itself is one-shot (no repeating timer)..."
...
echo "[5/6] Checking message text + no hardcoded farm wait..."
...
echo "[6/6] Checking version bump (informational) + no new independent switch..."
...
if [ "$control_ver" != "3.2.2" ]; then
    echo "⚠️ control 版本不是 3.2.2（实际：${control_ver:-空}）—— 请把 antforest/Package/DEBIAN/control 一并覆盖"
fi
...
echo "✅ All feed-wake (倒计时结束立即补喂) checks passed successfully!"
