...
echo "[8/8] Checking 多条日志多选复制（自绘选中态：点按切换绿勾 → 复制所选，最新在上顺序）..."
grep -Fq 'action:@selector(toggleLogSelectionMode:)' "$port_file"
grep -Fq 'toggleLogSelectionMode:(UIButton *)sender {' "$port_file"
grep -Fq 'NSMutableSet<NSNumber *> *selectedLogRows' "$port_file"
grep -Fq 'UITapGestureRecognizer *logTap' "$port_file"
grep -Fq 'action:@selector(handleLogTap:)' "$port_file"
grep -Fq 'logTap.cancelsTouchesInView = NO;' "$port_file"
grep -Fq 'handleLogTap:(UITapGestureRecognizer *)gesture {' "$port_file"
grep -Fq 'if (!self.logSelectionMode) return;' "$port_file"
grep -Fq '[self.selectedLogRows addObject:key];' "$port_file"
grep -Fq '[self.selectedLogRows removeObject:key];' "$port_file"
grep -Fq 'reloadRowsAtIndexPaths:@[indexPath] withRowAnimation:UITableViewRowAnimationNone' "$port_file"
...
grep -Fq '绿勾=已选' "$port_file"
grep -Fq '@"circle"' "$port_file"
grep -Fq 'BOOL picked = picking && [self.selectedLogRows containsObject:@(indexPath.row)];' "$port_file"
grep -Fq 'cell.backgroundColor = picked ? ' "$port_file"
if grep -Fq 'setEditing:' "$port_file" || grep -Fq 'allowsMultipleSelectionDuringEditing' "$port_file" || grep -Fq 'indexPathsForSelectedRows' "$port_file"; then
    echo "❌ 多选不得依赖 UIKit 编辑态私有控件（空圈不显选中态的根因），必须自绘选中态"
    exit 1
fi
