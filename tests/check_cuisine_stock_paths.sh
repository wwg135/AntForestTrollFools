...
echo "[1/13] Checking 库存识别落盘/读取（NSUserDefaults + 只记正数）..."
...
echo "[2/13] Checking 持有数扫描：只认 cuisineId + 数字字段，只记 >0..."
...
echo "[3/13] Checking 可喂清单 = 有库存的菜谱（按持有数降序，识别到的种类全量）..."
...
echo "[4/13] Checking 「服务端判无库存」→ 记进当天名单 + 换下一个..."
...
echo "[5/13] Checking 主动查库存口径（syncAnimalStatus + QUERY_CUISINE_LIST / SYNC_RESUME / 10 分钟节流）..."
...
echo "[6/13] Checking 库存过期先查一次再喂（起手 5 秒等回包）..."
grep -Fq 'manorCuisineStockStale(now)' "$source_file"
grep -Fq '5 * NSEC_PER_SEC' "$source_file"

echo "[7/13] Checking 识别到库存就投喂（60 秒监控 + 回包学习两条路）..."
...
echo "[8/13] Checking 投喂成功扣本地库存（扣到 0 从清单消失）..."
...
echo "[9/13] Checking 负向：高级饲料入口不得再因「正在吃」早退（吃普通饲料中照喂）..."
...
echo "[10/13] Checking 负向：4 秒无回包不再计成功、拉黑该菜谱后继续..."
...
case "$timeout_block" in
    *'[gManorCuisineBadIds addObject:stuckId];'*) ;;
...
echo "[12/13] Checking 无独立开关残留 + Makefile 挂载..."
...
grep -Fq '高级饲料本轮结算——成功 %lu 个，失败 %lu 个，判无库存 %lu 个' "$source_file"
grep -Fq '未投喂 %@' "$source_file"
grep -Fq 'BOOL roundRan = gManorCuisineRunning;' "$source_file"
grep -Fq 'if (roundRan) [self logManorCuisineRoundSummary];' "$source_file"
grep -Fq 'if (!roundRan) {' "$source_file"
grep -Fq 'gManorCuisineRoundOwned = manorOwnedCuisineList().count;' "$source_file"
grep -Fq -- '-(void)logManorCuisineRoundSummary;' "$header_file"
