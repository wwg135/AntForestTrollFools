...
## v3.5.0 芭芭农场 · 做美食（小鸡厨房）自动做满

进一次**芭芭农场**即自动执行（面板开关：``做美食（小鸡厨房）``，默认开）：

1. ① 拉厨房状态（``enterKitchen``）
2. ② 领每日食材 +60（``collectDailyFoodMaterial``）
3. ③ 农场食材成熟则收（``antorchard.farmFoodMaterialCollect``，``foodStatus=FINISHED`` 才收）
4. ④ 刷新可用次数；仍做不了且饲料 ≥60 时试领**每日限时食材 +10**（``collectDailyLimitedFoodMaterial``）
5. ⑤ 做满当天可做次数（``cook``，单轮上限 5，每次间隔 3.5s）
6. ⑥ 做菜后**重新拉状态**再清厨房垃圾（``collectKitchenGarbage``）——做菜会产生等量垃圾，必须用新状态判定
7. ⑦ 收尾校验并打印汇总
...
| 项 | 值 |
|---|---|
| 可做次数 | ``cookTimesAllowed = floor((厨房食材 + 食材仓)/60)``，做菜**同时扣两边** |
| 每次消耗 | 饲料 60g + 食材 60 |
| 每次产出 | 1 菜谱 + 肥料 ``manureAmount``（≈50~60）+ 等量厨房垃圾 |
| 食材三源 | 每日 60 / 每日限时 10 / 农场成熟可收 |
| 发往哪座桥 | 芭芭农场页桥即可（``com.alipay.antfarm.*`` 无需本页归属，实测通过） |

保护：同一次进入 60s 内不重复跑；每步等回包超时 15s；判包按**回包结构**认（cook 回包必带 ``cookResult``），不按「最近一次请求的 op」；回包外层随桥不同（``RVKJsBridge`` 是 ``{resData:{…}}``、``PSDJsBridge`` 平铺）⇒ 取值走**层级回退链**，缺键 ≠ 值为 0。

日志（面板可见）：``做美食：① … ⑦ …``、``状态· 初始/备料后/收尾``、``汇总：做美食 N 次…``。
